...
**Invariant** continuously tests your backend against real-world provider edge cases (duplicate deliveries, out-of-order lifecycle events, tampered HMAC signatures) and verifies database state invariants (`paymentCount == baseline + 1`).

---

## Quickstart (The Stripe Webhook Wedge)

Test your Stripe webhook integration locally in under 10 seconds:
...
============================================================
 SUMMARY: 4/4 Invariants Passed  (119ms)
...
module.exports = {
  ...
  provider: "stripe",
  webhookSecret: process.env.INVARIANT_WEBHOOK_SECRET || "whsec_xyz",
  ...
  invariants: [
    {
      ...
      description: "Duplicate webhook events must preserve single DB state record",
      expectHttp: 200,
      assertState: (state, httpRes, baseline) => state.paymentCount === baseline.paymentCount + 1
    },
    {
      ...
      description: "Invalid provider signature header must be rejected without mutating DB state",
      expectHttp: 401,
      assertState: (state, httpRes, baseline) => httpRes.status === 401 && state.paymentCount === baseline.paymentCount
    },
    {
      ...
      description: "Out-of-order refund events prior to payment must not corrupt state ledger",
      expectHttp: 200,
      assertState: (state, httpRes, baseline) => state.paymentCount === baseline.paymentCount && state.payments.every(p => p.status !== 'CORRUPTED')
    },
    {
      ...
      description: "Server 500 errors must be handled gracefully without inserting corrupt DB records",
      expectHttp: 500,
      assertState: (state, httpRes, baseline) => httpRes.status === 500 && state.paymentCount === baseline.paymentCount
    }
  ]
};
```
