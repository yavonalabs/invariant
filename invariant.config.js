...
module.exports = {
  ...
  targetUrl: process.env.INVARIANT_TARGET_URL || "http://localhost:3001/api/webhook",
  
  // State Assertion Probe Endpoint (Queries backend DB state)
  probeUrl: process.env.INVARIANT_PROBE_URL || "http://localhost:3001/api/db-state",

  // Optional Reset Endpoint (Resets DB state before each scenario)
  resetUrl: process.env.INVARIANT_RESET_URL || "http://localhost:3001/api/reset-state",
  
  // Payment Gateway Provider ('stripe' | 'razorpay')
  provider: process.env.INVARIANT_PROVIDER || "stripe",

  // Gateway Signing Secret
  webhookSecret: process.env.INVARIANT_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET || "whsec_yavona_secret_12345",
  ...
  invariants: [
    ...
    },
    {
      ...
      expectHttp: 401,
      assertState: (state, httpRes, baseline) => state.paymentCount === baseline.paymentCount
    },
    {
