#!/usr/bin/env node
/**
 * Invariant CLI Command Router (`npx invariant`)
...
${fmt.bold('COMMANDS:')}
  ${fmt.cyan('init')}                    Generate template ${fmt.bold('invariant.config.js')} in project
  ${fmt.cyan('test stripe-webhooks')}    Execute provider-accurate Stripe webhook state assertions
  ${fmt.cyan('test payment')}            Execute payment webhook state assertions
